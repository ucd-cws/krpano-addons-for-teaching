...
function Hotspot(panonum,hotspotid) {
	...
	this.loadpano = function(num) {
		num = getFileName() + (num - 1) + ".xml"; //filename == base_name in json file.
	...
	this.isCurrentPano = function() {
		return this.panoid == this.currentpanonum; //currentpanonum is global
	}
...
function Text(panonum, hotspotid) {
	...
	this.play = function() {
		this.checkCurrent();
		krpano().call("closeallobjects();set(plugin[" + hotspotid + "object].visible,true);"+
					  "tween(plugin[" + hotspotid + "object].alpha, 1);");
	}
}
...
function ClassData(thedata) {
	...
	var addhotspot = function(h, enable_icons) {
		...
		idToIndex[h.id] = myhotspots.length;
	...
	var panostops = function(places) {
		    ...
		    idToIndex[pano] = myhotspots.length;
			var fun2 = function(){loadPanoNum(places.pano_num)}
			myhotspots.push(fun2);
	...
	var hasNext = this.hasNext = function() {
		return index < myhotspots.length - 1;
	}
